...
function loadCenterContent(url, clickedLink = null) {
  fetch(url)
    ...
    .then(html => {
      // Update the center div with the loaded HTML content
      ...
      if (clickedLink) {
        ...
        clickedLink.style.pointerEvents = "none"; // Disable the current link
      }

      // After loading the new content, initialize music list if it's the "songs" page
      if (url === 'songs.html' || url.includes('songs')) {
        initializeMusicList();
      }
    })
    .catch(err => {
...
// Function to initialize music list when the Songs page is loaded
function initializeMusicList() {
  ...
  fetch('songs.json')
    ...
    .then(musicList => {
      // Clear any existing content in the music list container
